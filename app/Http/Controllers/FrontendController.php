...
class FrontendController extends Controller
{
  ...
  public function all_post(){
    ...
    $post = post::with('location','account','Tag')->orderBy('created_at','ASC')->paginate(9);
    // dd($post);
  ...
  //  dd($similarPosts);
    ...
    // dd($popularPost);
    // if($post){
      return view('website.post',compact('post','popularPost','comment'));
    // }else{
    //   return redirect('/');
    // }
    ...
    public function send_comment(Request $request, $postId)
    {
        // Sử dụng $postId để tìm thông tin về bài viết hiện tại
    ...
    // dd($postId);
            ...
            $comment = Comment::create([
                ...
                'post_id' => $postId, // Liên kết với id của bài viết hiện tại
            ]);
            
  


        Session::flash('comment-send','Comment send successfully');
        return redirect()->back();
    }
}